    ...
    */
    ...
    public Solveur(double[][][] volt, double[][][] curr, double[][][] adm,double[] cg, Equation[] eq) {
        ...
        */
        ...
        //###############################################################################################################tester la validité des données en entrée : taille des tableaux, format des admittances, symetrie des courants tension, etc..
    ...
    private boolean replace(int[] id,int cat, int nb, double value) {//cat and nb give the original equation, so that we do not modify it.
        //##############################################################################################################verifier qu'on a pas déja déterminé cette valeur
    ...
    public boolean set_variable_value(int[] id, double value,boolean remplissage) {/////////////////remplacer les valeurs dans les equations
        ...
        if (variables[id[0]][id[1]][id[2]][0] == 1) {// if the value is already determined
            if (variables[id[0]][id[1]][id[2]][1] == value) {
                if (!remplissage);// true;////////////////////////////////////////////////////////retourner une erreur d'équations equivalentes
            }
            else return false;
        }
    ...
    private boolean make_symetries() {
        for (int i_t=0;i_t<3;i_t++){
            for (int i=0;i<nb_nodes;i++) {
                for(int j=0;j<=i;j++) {
                    if (i == j) {
                        if (variables[i_t][i][j][1] != 0) return false;/////////////////////////////////////////exception parametre pas à 0
                        continue;
                    }
                    else {
                        if ((variables[i_t][i][j][0] == 1) && (variables[i_t][j][i][0] == 0)) {//if only [i,j] is known
                            if (i_t == 2) variables[i_t][j][i] = new double[]{1, variables[i_t][i][j][1]};
                            else variables[i_t][j][i] = new double[]{1, -1*variables[i_t][i][j][1]};
                            continue;
                        }

                        if ((variables[i_t][j][i][0] == 1) && (variables[i_t][i][j][0] == 0)) {//if only [j,i] is known
    ...
    private boolean substituate_variable() {
        ...
        double[] param = equations[ind].get_value(indice);//constante, coefficient du courant générateur
            ...
            if (!equations[cpt].used) {
                if (equations[cpt].stable) {
                    if (!equations[cpt].trivial) {
                        if ((equations[cpt].is_current_present()) || (!cur)) {//if a current is substituable or if we have determined or substituated all of them
                            i = cpt;
                            break;
                        }
                    }
    ...
    public boolean update_number_unknown() {//returns if all variables are found
        int nb = 0;
